...
async function sortCreatedList(){
    var createdBtn = document.querySelector(".css-1078jy");
    createdBtn.click();
    await new Promise(resolve => setTimeout(resolve, 4000));
    createdBtn = document.querySelector(".css-1078jy");
    createdBtn.click();
    await new Promise(resolve => setTimeout(resolve, 4000));
}

async function generateReport(isRMreport) {
    await sortCreatedList();
    ...
    for (var i = 0; i < 35; i++) {
        ...
        try {
            if (eachTicket && eachTicket.classList.contains("_4t3i2nrh")) {
                ...
                if (eachCol[2]) {
                    var dateCreated = eachCol[2].firstChild.firstChild.innerText;
...
console.log("\n");
for(var i = 0; i<6 ; i++ ){
    var rtnMailTotalCount = rtrnMailPolicyList[monthsArray[currentDate.getMonth() - i]].size;
    var adrsRcvdTotalCount = rcvdAdrsList[monthsArray[currentDate.getMonth() - i]].size;
    var total = rtnMailTotalCount + adrsRcvdTotalCount;
    console.log("In ", monthsArray[currentDate.getMonth() - i], " we received total ", adrsRcvdTotalCount, " new addresses out of ", rtnMailTotalCount, " return mails.")
}
console.log("\nList of Policies whose address we never received: ",rtrnMailPolicyList);
...
console.log("\nTotal number of policies with DONOT mail: ", countTotalMails(rtrnMailPolicyList), "(Meaning we didn't receive new address for these policies)");
